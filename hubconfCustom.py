          ...
          for i, det in enumerate(pred):
            ...
            if len(det):
              ...
              for *xyxy, conf, cls in reversed(det):
                ...
                label_noconf = f'{names[int(cls)]}'             #removed confidence (updated by akshat) 
                list_label.append(label_noconf)           #appending all labels found to list_label (updated by akshat)
 
                plot_one_box(xyxy, img0, label=label, color=colors[int(cls)], line_thickness=3)
          fps_x = int((j+1)/(time.time() - start_time))
          # print(f"{j+1}/{nframes} frames processed")
          # print(conf)
          yield img0, fps_x, img0.shape, total_detections, list_label           #list_label added manually by akshat
